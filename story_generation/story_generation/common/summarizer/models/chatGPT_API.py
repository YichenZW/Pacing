...
class ChatGPT3Summarizer(AbstractSummarizer):
    def __init__(self, args):
        ...
        self.logger = None # just directly print.
                ...
                try:
                    ...
                    if context_length > self.args.max_context_length:
                        if self.logger is None:
                            print('context length' + ' ' + str(
                            context_length) + ' ' + 'chatgpt exceeded artificial context length limit' + ' ' + str(
                            self.args.max_context_length))
                        else:
                            self.logger.warning('context length' + ' ' + str(
                            context_length) + ' ' + 'chatgpt exceeded artificial context length limit' + ' ' + str(
                            self.args.max_context_length))
                        time.sleep(1)  # similar interface to gpt3 query failing and retrying
                        assert False
                    ...
                    if retry:
                        if self.logger is None:
                            ...
                            self.logger.warning(f'retrying... sleeping {num_fails} seconds...')
