...
def get_token_numbers(t):
    return len(gpt2_tokenizer.encode(t))
def rand_throw(text, target_len):
    #sentences = tokenizer.encode(text, add_special_tokens=False, truncation=True)
            ...
            while retry:
                try:
                    ...
                    if context_length > self.args.max_context_length:
                        logging.warning('gpt3 context length' + ' ' + str(context_length) + ' ' + 'exceeded artificial context length limit' + ' ' + str(self.args.max_context_length))
                        prompt = rand_throw(prompt, self.args.max_context_length)
