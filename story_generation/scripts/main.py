...
from story_generation.common.summarizer.models.chatGPT_API import ChatGPT3Summarizer
...
if __name__=='__main__':
    ...
    parser = add_summarizer_args(parser) # generation model is here in --gpt3-model
    parser = add_controller_args(parser) # generation model is here in --controller-model-string
    ...
    gpt3_model = GPT3Summarizer(args)
    ...
    # opt_model = ChatGPT3Summarizer(args)
    ...
    if args.save_complete_file is not None:
        with open(args.save_complete_file, 'wb') as wf:
            for b in beam:
                b.controllers = None
            pickle.dump(beam, wf)
        with open(args.save_complete_file+".final.txt", "w") as file:
            file.write(beam[0].story())
        print("Final story has been written to", args.save_complete_file+".final.txt")
